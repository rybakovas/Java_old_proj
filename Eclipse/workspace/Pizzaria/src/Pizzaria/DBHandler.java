...
try {
			...
			for (String s : escolhecardapio)
			{
				...
				ResultSet rs = sst.executeQuery("Select idProduto From poo_pw_pizzaria.produto where Descr_Prod='"+s+"';");
				rs.next();
				String  Id_Prod =rs.getString("idProduto");
	...
	public ArrayList<String> getPedido() {
		...
		try {
			...
			ResultSet rs = sst.executeQuery("Select poo_pw_pizzaria.produto.Descr_Prod from  "
					...
					+ "where poo_pw_pizzaria.pedido.Cliente_idCliente='"+ this.id +"' "
					+ "and poo_pw_pizzaria.pedido.Status_Ped='wait';");
